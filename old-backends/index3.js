...
async function checkCalendarAvailability(date, time, instructorId) {
    try {
        ...
        // If we can't check the calendar, assume it's available but log the error
        return {
            isAvailable: true,
...
// Function to get calendar context for AI
...
async function getSystemPrompt(instructorId) {
    ...
    const busySlotsText = busySlots.length > 0 
        ? `\n\nCURRENTLY BOOKED TIME SLOTS (NOT AVAILABLE):\n${busySlots.map(slot => `${slot.date} at ${slot.time}`).join('\n')}`
...
- Available times: 9:00 AM, 10:00 AM, 11:00 AM , 12:00 PM , 2:00 PM, 3:00 PM, 4:00 PM
