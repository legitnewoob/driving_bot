...
console.log = (...args) => {
  ...
  // This is your line 38 that was causing the error
  if (!logStream.closed) {
...
class GeminiTester {
  ...
  async testBasicResponses() {
    ...
    const basicTests = [
      ...
      },
      {
        ...
        expectContains: ["driving", "lessons", "book" , "cancel" , "update"],
        expectAction: "NULL"
      },
      {
        message : "Can you provide me with information on how to tackle roundabouts?",
        expectContains: ["roundabout" , "driving"],
        expectAction: "NULL"
      },
      {
        message : "What's the earliest available slot?",
        expectContains: ["available", "slot", "lesson" , "book"],
        expectAction: "NULL"
      }
  ...
  async testBookingScenarios() {
    ...
    const bookingTests = [
      ...
      },
      {
        ...
        expectAction: "NULL",
        expectContains: ["unavailable", "weekdays", "9am-4pm" , "next week" , "working days"],
        description: "Booking outside working hours"
      },
      {
        message: "Book a lesson for 23rd",
        expectAction: "NULL",
        expectContains: ["september", "time", "when"],
  ...
  async testActionExtraction() {
    ...
    const actionTests = [
      {
        response: "I'm processing your booking now... [ACTION:BOOK]\n{\"date\": \"2024-03-15\", \"time\": \"14:00\"}",
        expectAction: "book",
  ...
  async testErrorHandling() {
    ...
    try {
      ...
      this.log(`✅ Empty message handled gracefully: ${response.substring(0, 50)}...`, 'green');
    ...
    try {
      ...
      if (test.expectAction) {
        ...
        if (actualAction !== expectedAction) {
          this.log(`❌ Expected action: ${expectedAction}, got: ${actualAction}`, 'red');
          passed = false;
        } else {
  ...
  async runDateTimeTest(test) {
    ...
    try {
      ...
      const result = await DateTimeService.extractDateTimeFromMessage(test.message);
      ...
      if (test.expectDateTime !== result.hasDateTime) {
        this.log(`❌ Expected hasDateTime: ${test.expectDateTime}, got: ${result.hasDateTime}`, 'red');
        passed = false;
      } else {
      ...
      if (test.expectTime && result.time !== test.expectTime) {
        this.log(`❌ Expected time: ${test.expectTime}, got: ${result.time}`, 'red');
        passed = false;
      ...
      if (test.expectDate && result.date !== test.expectDate) {
        this.log(`❌ Expected date: ${test.expectDate}, got: ${result.date}`, 'red');
        passed = false;
  ...
  runActionExtractionTest(test) {
    ...
    try {
      ...
      if (result.actionType !== test.expectAction) {
        this.log(`❌ Expected action: ${test.expectAction}, got: ${result.actionType}`, 'red');
        passed = false;
      } else {
      ...
      if (test.expectData && JSON.stringify(result.bookingData) !== JSON.stringify(test.expectData)) {
  ...
  printSummary() {
    ...
    if (this.testResults.failed === 0) {
      this.log('\n🎉 All tests passed! Your Gemini integration is working perfectly!', 'green');
    } else {
      this.log(`\n⚠️  ${this.testResults.failed} test(s) failed. Check the output above for details.`, 'yellow');
    }
...
if (require.main === module) {
  ...
  if (args.includes('--quick')) {
    ...
    tester.testBasicResponses();
  } else if (args.includes('--booking')) {
    tester.testBookingScenarios();
  } else if (args.includes('--extraction')) {
    tester.testDateTimeExtraction();
  } else if (args.includes('--actions')) {
    tester.testActionExtraction();
